//From Malik, "C++ Programming: Program Design Including Data Structures" ed.5 textbook files online at course.com/malik/cpp.
...
//@PARAM:type for [[nodeType<Type>* insert(short type, const Type& insertItem);]]
#define INSERT_END 1
#define INSERT_SORTED 2
//@PARAM:type for [[void delete_nodeType(short type, nodeType<Type>* deleteItem);]]
#define DELETE_FORCE 1
  ...
  class doublyLinkedList
{
  public:
    ...
    //modified...
    nodeType<Type>* insert(short type, const Type& insertItem);
    ...
    //
    //Added: --if type=INSERT_SORTED, insert into ordered list
    //       --if type=INSERT_END, insert at end of list
    //       --the function returns inserted nodeType
    //
    ...
    //added...
    void delete_nodeType(short type, nodeType<Type>* deleteItem);
    //Function to delete address of deleteItem from the list. 
    //Postcondition: The item addressed by deleteItem is deleted.
    //               If type=DELETE_FORCE, for efficiency purposes, 
    //                         there is no check whether deleteItem
    //                         is in the list.  The item is deallocated
    //                         from memory.  Be sure that deleteItem
    //                         exists in the list!!!!! 
    //               If type=DELETE_CHECK, delete the item addressed
    //                         by deleteItem if and only if it exists.  
    //
  ...
  doublyLinkedList<Type>::doublyLinkedList()
{
  ...
  void doublyLinkedList<Type>::print() const
  {
    ...
    {
      // modified...
      cout << "[@" << current << ":" << current->info << "]";  //output info
      //
  ...
  template <class Type>
  nodeType<Type>* doublyLinkedList<Type>::insert(short type,const Type& insertItem)
  {
    nodeType<Type> *current;      //pointer to traverse the list
    ...
    else
    {
      ...
      switch(type)
      {
        ...
        case INSERT_SORTED:
          //
          // previous version; add in sorted list
          //
...
//added...
  template <class Type>
  void doublyLinkedList<Type>::delete_nodeType(short type, nodeType<Type>* deleteItem)
  {
    nodeType<Type> *current; //pointer to traverse the list
    nodeType<Type> *trailCurrent; //pointer just before current
    ...
    if (first == NULL)
      cout << "Cannot delete from an empty list." << endl;
    else if (first == deleteItem) //node to be deleted is  
      //the first node
    {
      current = first;
      first = first->next;
    ...
    else 
    {
      ...
      if(type==DELETE_FORCE) // delete with force
      {
        current=deleteItem;
      }else // check before deleting
      {  
        current = first;
        
        while (current != NULL && !found)  //search the list
          if (current == deleteItem)
          found = true;
        else
          current = current->next;
      }
      if (current == NULL) {
        //   cout << "The item to be deleted is not in " 
        //       << "the list." << endl;
      }else if (current == deleteItem) //check for 
        //equality
      {
        trailCurrent = current->back; 
        trailCurrent->next = current->next;
